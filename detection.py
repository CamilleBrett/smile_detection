import cv2
# https://towardsdatascience.com/facial-recognition-happiness-bbb3c4293d1d


# Load cascades
# OpenCV fournit des algorithmes de cascades Haar pré-entrainés
cascade_face = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
cascade_eye = cv2.CascadeClassifier('haarcascade_eye.xml')
cascade_smile = cv2.CascadeClassifier('haarcascade_smile.xml')

# Define the detection function on face, eyes and smile
...
def detection(grayscale, img):  # we work on the grayscale pic but also want to show the rvb pic
    ...
    for (x_face, y_face, w_face, h_face) in face:
        # draw the triangles : x,y : upper hand corner; w,h : height and width
        cv2.rectangle(img, (x_face, y_face), (x_face+w_face,
                                              y_face+h_face), (255, 130, 0), 2)
        # Interest region
